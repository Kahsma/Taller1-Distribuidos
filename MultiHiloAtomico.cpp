...
mutex passwordMutex;
atomic<bool> stopThreads(false); // Atomic flag to signal threads to stop
...
void findPasswordInRange(const string &hashToMatch, const string &filename, int startLine, int endLine, string &foundPassword)
{
    ...
    string line;
    int lineCounter = 0;
    while (getline(file, line) && !stopThreads)
    {
        if (lineCounter >= startLine && lineCounter <= endLine && hashMatch(line, hashToMatch))
        {
            {
                lock_guard<mutex> lock(passwordMutex);
                if (!foundPassword.empty()) // Another thread already found the password
                {
                    break;
                }
                foundPassword = line;
                stopThreads = true; // Signal other threads to stop
            }
            break;
        }
        lineCounter++;
